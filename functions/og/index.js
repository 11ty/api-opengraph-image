...
async function handler(event, context) {
  ...
  try {
    ...
    let stats = await og.getImage(imageFormat || FALLBACK_IMAGE_FORMAT, maxWidth);
    console.log( "Generated", url, stats );
    let format = Object.keys(stats).pop();
    let stat = stats[format][0];
    console.log( "Found match", url, stat );
    ...
    return {
      ...
      statusCode: 200,
      // HOWEVER HOWEVER, we can set a ttl of 60 which means that the image will be re-requested in 60 seconds.
      ttl: 60,
      headers: {
