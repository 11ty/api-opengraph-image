...
function getErrorImage(message, statusCode, ttl, cacheBuster) {
  ...
  // we want an <img onerror> to trigger on a 404 and that doesn’t happen with image content
  if(statusCode !== 404) {
...
async function handler(event, context) {
  ...
  try {
    ...
    if(!imageUrls.length) {
      ...
      // Both 301 and 302 redirects in image sources are not reflected in clientside .src or .currentSrc
